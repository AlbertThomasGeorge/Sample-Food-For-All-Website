...
<body>
    <h1 class="h1-heading">Change Password:- Delivery Person</h1>
        ...
        <?php
            if(isset($_POST['save-change'])){
                ...
                if($changedpassword == $confirmchangedpassword){     
                    $sql1 = "SELECT `password` FROM delivery_people WHERE delivery_person_id = {$deliverypersonid}";
                    ...
                    if($result1){
                        ...
                        if($row1["password"] == $changedpassword){
                            ...
                            $sql2 = "UPDATE delivery_people SET changed_password = '{$changedpassword}' WHERE delivery_person_id = {$deliverypersonid}";
                            if(mysqli_query($conn, $sql2)){
                                mysqli_close($conn);
                                header("Location: http://localhost/MINI-PROJECT/delivery-person-login.php");
                            }
                        }
                    }
                }
