...
<head>
    ...
    <link rel="stylesheet" href="../css/orders-from-not-restaurant.css">
</head>
<body>
    ...
    <div id="submit-date">
        ...
        <button>Get Orders From Not Restaurant</button>
    </div>
    <div class="error"></div>
    <div id="orders-from-not-restaurant"></div>
    ...
    <script>
        $(document).ready(function(){
                              ...
                              function loadOrdersFromNotRestaurant(date, order_identifier){
                                  $.ajax({
                                              url: "load-orders-from-not-restaurant.php",
                                              type: "POST",
                                              data: {date_input: date, order_id: order_identifier},
                                              success: function(data){
                                                           $('#orders-from-not-restaurant').html(data);
                                                       }
                                  });
                              }
                              $('button').click(function(){
                                                    ...
                                                    if(date == ''){
                                                        ...
                                                        $('.error').html('').hide();
                                                        loadOrdersFromNotRestaurant(date);
                                                    }
                                                });
                              $('input').on("input", function(){
                                                         load_repeatedly = false;
                                                         $('#orders-from-not-restaurant').html('');
                                                     });
                              $(document).on("click", ".prepared", function(){
                                                                       let order_id = $(this).data('order_id');
                                                                       loadOrdersFromNotRestaurant(date, order_id);
                                                                   });
