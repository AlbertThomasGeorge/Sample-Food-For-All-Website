...
?>
<body>
    <h1 class="h1-heading">Restaurant Staff First Time Login Page</h1>
    <div class="login-container">
        <?php
            if(isset($_POST['login'])){
                ...
                $sql = "SELECT restaurant_staff_id FROM restaurant_staff WHERE restaurant_staff_name = '{$restaurantstaffname}' AND `password` = '{$password}'";
                $result = mysqli_query($conn, $sql) or die("Query Failed");
                if(mysqli_num_rows($result)>0){
                    while($row=mysqli_fetch_assoc($result)){
                        $restaurantstaffid = mysqli_real_escape_string($conn, $row['restaurant_staff_id']);
                        mysqli_close($conn);
                        header("Location: http://localhost/MINI-PROJECT/restaurant-staff-change-password.php?restaurantstaffid={$restaurantstaffid}");
                    }
                }
                else{
